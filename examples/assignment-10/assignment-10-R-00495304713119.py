...
def process(s):
    ...
    while i < len(s):
        if s[i] in diff:
            local = diff.index(s[i])
            dic[local][1].append(i)
        else:
            diff.append(s[i])
            dic.append([s[i], [i]])
        i += 1
...
if __name__ == '__main__':
    ...
    assert occurrences(process('mississippi'), ['s', 'i']) == [[2, 3, 5, 6],
        [1, 4, 7, 10]]
    print('stats(gdp):', stats(gdp))
    print('histogram(le):', histogram(le))
    print('quartiles(gdp):', quartiles(gdp))
    print("occurrences of 's' ,'i' in 'mississipi'=", occurrences(process(
        'mississippi'), ['s', 'i']))
