...
@click.argument('path_to_submissions_directory', nargs=1, default='.', type=click.Path(exists=True))
@click.option('--verbose', '-v', is_flag=True)
def mclust(path_to_submissions_directory, verbose):
...
@click.argument('path_to_submissions_directory', nargs=1, default='.', type=click.Path(exists=True))
@click.option('--max-distance', '-d', help='Set a maximum in-cluster cophenetic distance.', type=float)
@click.option('--num-clusters', '-n', help='Set the number of preferred clusters.', type=int)
@click.option('--verbose', '-v', is_flag=True)
def cluster(path_to_submissions_directory, max_distance, num_clusters, verbose):
    ...
    if num_clusters is not None:
        if max_distance is not None:
            raise click.ClickException('Can only use one of -d and -n (max distance or number of clusters.')
        clustering = fcluster(linkage_matrix, num_clusters, 'maxclust')
    elif max_distance is not None:
        clustering = fcluster(linkage_matrix, max_distance, 'distance')
    else:
        raise click.ClickException('Must provide either a max distance or a set number of clusters (using -d or -n flags).')
...
def clustrep(path_to_submissions_directory, cluster_number, human_in_the_loop):
    ...
    cluster_neighbor_cluster = [cluster_number for (cluster_number, cluster_members) in clusters.items() if cluster_neighbor in cluster_members][0]
    click.echo(' Cluster representative: '.center(80, '#') + '\n')
    click.echo('File name: {} (assignment {})\nAverage distance: {}\n'.format(basename(submissions[cluster_median]), cluster_median, cluster_median_avg_dist))
    hloop.print_submission(submissions[cluster_median])
    click.echo()
    click.echo(' Most distant cluster member: '.center(80, '#') + '\n')
    click.echo('File name: {} (assignment {})\nAverage distance: {}\n'.format(basename(submissions[cluster_outlier]), cluster_outlier, cluster_outlier_avg_dist))
    ...
    if cluster_neighbor is not None:
        ...
        click.echo('File name: {} (assignment {})\nCluster: {}\nAverage distance: {}\n'.format(basename(submissions[cluster_neighbor]), cluster_neighbor, cluster_neighbor_cluster, cluster_neighbor_avg_dist))
